...
- 正式 DMG：`dist/Y-Dock-vX.Y.Z-arm64.dmg` 与 `dist/Y-Dock-vX.Y.Z-x86_64.dmg`
...
- 自动更新必须按编译架构精确匹配完整资产名 `Y-Dock-vX.Y.Z-arm64.dmg` 或 `Y-Dock-vX.Y.Z-x86_64.dmg`，并在替换前严格验证下载 App 主可执行文件是当前编译架构唯一 slice 的 thin binary；缺少或错配时只能安全失败并引导 Release 页面，不得选择其他 DMG 或删除现有 App。
