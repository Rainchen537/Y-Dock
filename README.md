...
| 🚦 桌面窗口红绿灯 | 在其他 App 的普通桌面窗口左上角提供自有红黄绿控制层；可让按钮平时隐藏、进入控制区域后显示，并只放大当前悬浮的单颗按钮。 |
...
6. 可在设置中启用桌面窗口红绿灯增强：按钮可平时隐藏，鼠标进入目标窗口左上控制区域后显示；悬浮某一颗时只放大该按钮。红色按钮可按策略关闭当前窗口或请求正常退出所属 App。
...
- `桌面红绿灯显示`：可让其他 App 普通桌面窗口左上角的红黄绿按钮默认隐藏，仅在鼠标进入对应控制区域后显示。
...
macOS 没有公开 API 可直接改变其他进程原生红黄绿按钮的尺寸或可见性。桌面窗口红绿灯增强通过 Y-Dock 自有的非激活覆盖层遮住并重绘目标按钮，只对能可靠匹配 CGWindow 与标准 AXWindow/控制按钮的普通窗口生效；匹配不明确、窗口已全屏或动作前位置已变化时会保守跳过。
